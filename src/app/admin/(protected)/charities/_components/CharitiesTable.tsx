...
import { CHARITY_CATEGORY_LABEL, CHARITY_CATEGORY_OPTIONS } from '@/lib/utils/charityCategories'

const CATEGORY_RANK: Record<string, number> = Object.fromEntries(
  CHARITY_CATEGORY_OPTIONS.map((o, i) => [o.value, i])
)

export default function CharitiesTable({ charities }: { charities: Charity[] }) {
  ...
  const sorted = useMemo(() => {
    const copy = charities.slice()
    copy.sort((a, b) => {
      const catCmp = CATEGORY_RANK[a.category] - CATEGORY_RANK[b.category]
  ...
  if (charities.length === 0) {
    return (
      <div className="evt-empty">
        <p>No charities yet. Click &ldquo;+ Add New Charity&rdquo; to create the first one.</p>
        <CharitiesTableStyles />
      </div>
    )
  }
  ...
  return (
    <div className="evt-wrap">
      ...
      <table className="evt-table">
        ...
        <tbody>
          {sorted.map((c) => (
            <tr key={c.id} className="evt-row">
              <td>
                <Link href={`/admin/charities/${c.id}/edit`} className="evt-title-link">
                  {c.name}
                </Link>
              </td>
              <td>{CHARITY_CATEGORY_LABEL[c.category] ?? c.category}</td>
              <td className="evt-mono">{c.sort_order}</td>
              <td>
                {c.is_published ? (
              ...
              <td className="evt-actions">
                <div className="evt-actions-inner">
                  <Link href={`/admin/charities/${c.id}/edit`} className="evt-action-link">Edit</Link>
                  <button
